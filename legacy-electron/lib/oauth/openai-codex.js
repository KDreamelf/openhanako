...
function extractImportedTokens(parsed) {
  const candidates = [
    parsed?.tokens,
    parsed?.credentials,
    parsed?.oauth?.tokens,
  ...
  for (const candidate of candidates) {
    ...
    const access = pickStringField(candidate, [
      "access_token",
      "accessToken",
      "access",
      "token",
    ]);
    const refresh = pickStringField(candidate, [
      "refresh_token",
      "refreshToken",
      "refresh",
    ]);
    const accountId = pickStringField(candidate, [
      "account_id",
      "accountId",
    ]);
...
export function parseOpenAICodexAuthFileContent(content) {
  ...
  const { access, refresh, accountId: explicitAccountId } = extractImportedTokens(parsed);
  if (!access && !refresh && parsed?.OPENAI_API_KEY) {
    throw new Error("当前 auth.json 只有 OPENAI_API_KEY，缺少 ChatGPT Plus/Pro 登录态 token");
  }
  if (!access || !refresh) {
    throw new Error("认证文件缺少 access_token 或 refresh_token");
  }
  if (parsed?.auth_mode && parsed.auth_mode !== "chatgpt") {
    log.warn(`importing Codex auth.json with non-chatgpt auth_mode=${parsed.auth_mode}`);
  }
  ...
  const accountId = explicitAccountId || resolveImportedAccountId(parsed?.tokens) || undefined;
