 ...
 * @param {{ mode?: "smart"|"custom_git", git_dir?: string }} [opts.bash]
 * @returns {{ tools: object[], customTools: object[] }}
 */
export function createSandboxedTools(cwd, customTools, { agentDir, workspace, hanakoHome, mode, bash }) {
  ...
  if (policy.mode === "full-access") {
    ...
    const bashTool = isWin32
      ? createBashTool(cwd, { operations: { exec: createWin32Exec({ bashPreference: bash }) } })
      : createBashTool(cwd);
    return {
      tools: [
  ...
  if (platform === "win32-full-access") {
    const win32BashOps = { exec: createWin32Exec({ bashPreference: bash }) };
    return {
      tools: [
