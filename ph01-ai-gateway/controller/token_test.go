...
func TestPH01DefaultTokenCannotExposeOrDeleteKey(t *testing.T) {
	db := setupTokenControllerTestDB(t)
	if err := db.AutoMigrate(&model.PH01Identity{}); err != nil {
	...
	keyCtx.Params = gin.Params{{Key: "id", Value: strconv.Itoa(token.Id)}}
	GetTokenKey(keyCtx)
	keyResponse := decodeAPIResponse(t, keyRecorder)
	if keyResponse.Success {
		t.Fatalf("expected PH01 default key fetch to fail")
	}
	if strings.Contains(keyRecorder.Body.String(), token.Key) {
	...
	if !updateResponse.Success {
		t.Fatalf("expected PH01 default key group update to succeed, got %s", updateResponse.Message)
	}
