...
__device__ void sha512_hash(const uint8_t *msg, uint32_t len, uint8_t out[64]) {
    uint64_t state[8] = {
    ...
    for (uint32_t i = 0; i < rem; ++i) block[i] = msg[offset + i];
    block[rem] = 0x80;
    if (rem >= 112) {
        sha512_compress(state, block);
        #pragma unroll
    ...
    #pragma unroll
    for (int i = 0; i < 8; ++i) store_be64(out + i * 8, state[i]);
}
...
__device__ void hmac_sha512(
    ...
    if (key_len > 128) {
        sha512_hash(key, key_len, hashed_key);
        #pragma unroll
        for (int i = 0; i < 64; ++i) key_block[i] = hashed_key[i];
    } else {
    ...
    for (uint32_t i = 0; i < msg_len; ++i) inner[128 + i] = msg[i];
...
__device__ void pbkdf2_sha512_mnemonic(
    ...
    for (int iter = 2; iter <= 2048; ++iter) {
        ...
        hmac_sha512(password, password_len, prev, 64, u);
        #pragma unroll
        for (int i = 0; i < 64; ++i) out[i] ^= u[i];
    }
}

extern "C" __global__ void pbkdf2_sha512_kernel(
    const uint8_t *input,
    ...
    if (idx >= count) return;
    const uint8_t *password = input + offsets[idx];
    uint8_t seed[64];
    pbkdf2_sha512_mnemonic(password, lengths[idx], seed);
    #pragma unroll
    for (int i = 0; i < 64; ++i) output[idx * 64 + i] = seed[i];
}
